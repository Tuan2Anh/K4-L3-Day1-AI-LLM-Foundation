...
def compare_models(prompt: str) -> dict:
    ...
    mini_cost_estimate = (len(mini_text.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o-mini"]["output"]
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    import tiktoken
    try:
        # Lấy bộ mã hóa (encoding) chuẩn của model
        enc = tiktoken.encoding_for_model(model)
        # Mã hóa đoạn text thành mảng các token, rồi đếm số lượng bằng hàm len()
        return len(enc.encode(text))
    except Exception:
        # Phương án dự phòng (fallback): trung bình 1 token ≈ 4 ký tự
...
def run_assistant(
    ...
    # Bạn phải nhớ import OpenAI và khởi tạo client ở đây nhé (nếu chưa có)
    from openai import OpenAI
    client = OpenAI()
    ...
    while True:
        ...
        # Gom các messages lại theo đúng thứ tự
        messages = [{"role": "system", "content": persona}] + history \
                   + [{"role": "user", "content": user_msg}]
        
        # Gọi API có streaming, bọc trong hàm retry
        ...
        # Đọc stream, in ra màn hình và ghép lại thành bot_reply
        bot_reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            bot_reply += delta
        ...
        # Cập nhật lịch sử và xén bớt còn 3 lượt (6 dòng)
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": bot_reply})
        history = history[-6:]
        
        # Cập nhật các biến đếm và tính tiền
        num_turns += 1
        stats = estimate_cost(user_msg, bot_reply, OPENAI_MODEL)
